...
def get_latest_version(versions):
    ...
    latest = sorted(
        versions.items(),
        key=lambda x: (
            x[1]["manifest"]["versionCode"] if "versionCode" in x[1]["manifest"] else 0
        ),
        reverse=True,
    )[0]
...
def get_package_info(metadata, version_info, repository_url):
    ...
    return {
        ...
        "permissions": [
            p["name"] for p in manifest.get("usesPermission", []) if isinstance(p, dict)
        ],
    ...
    for repo_dir in os.listdir(cache_dir):
        ...
        for package_id, package_data in index_data.get("packages", {}).items():
            ...
            package_info = get_package_info(
                package_data["metadata"], latest_version, repository_url
            )
            row = {
                "repository": repo_dir,
                ...
                "name": name,
                "summary": get_localized_text(
                    package_data["metadata"].get("summary", "N/A")
                ),
                "description": get_localized_text(
                    package_data["metadata"].get("description", "N/A")
                ),
                "license": package_data["metadata"].get("license", "N/A"),
                "categories": json_enc.encode(
                    package_data["metadata"].get("categories", [])
                ),
                ...
                "donation_url": json_enc.encode(
                    package_data["metadata"].get("donate", [])
                ),
